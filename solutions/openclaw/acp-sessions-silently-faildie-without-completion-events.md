# ACP sessions silently fail/die without completion events

## 증상
ACP sessions spawned via `sessions_spawn(runtime='acp')` frequently die without emitting any completion or error event back to the parent session. The parent agent has no way to know the task failed — it just never hears back.

에러 메시지:
` frequently die without emitting any completion or error event back to the parent session. The parent agent has no way to know the task failed — it just never hears back.

In my testing (OpenAI Codex

## 원인
원본 이슈에서 확인 필요. GitHub Issue #52452 참조.

## 해결법
이 이슈의 해결법은 원본 GitHub Issue를 참조하세요.

## 예상 토큰 절약
이 에러로 삽질 시: 약 5,000~15,000 토큰 소비
이 해결법 참조 시: 약 500 토큰

## 환경
- OpenClaw 버전: 해당 이슈 시점 기준
- 관련 스킬/도구: openclaw
- OS: 다양

## 출처
https://github.com/openclaw/openclaw/issues/52452
