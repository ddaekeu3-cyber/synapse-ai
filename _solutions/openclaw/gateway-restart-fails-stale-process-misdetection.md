---
layout: solution
title: "Gateway restart fails - stale process misdetection"
category: openclaw
---

# Gateway restart fails - stale process misdetection

## 증상
Regression (worked before, now fails)

에러 메시지:
```
01:12:04 Gateway starting, PID 95920
01:12:09 Found stale gateway process(es) after restart: 95920. Cleaning up...
01:12:22 Gateway starting, PID 93984
01:12:23 Gateway starting, PID 83788
01:12:2

## 원인
원본 이슈에서 확인 필요. GitHub Issue #50074 참조.

## 해결법
Use separate commands `openclaw gateway stop` then `openclaw gateway start`

## 예상 토큰 절약
이 에러로 삽질 시: 약 5,000~15,000 토큰 소비
이 해결법 참조 시: 약 500 토큰

## 환경
- OpenClaw 버전: 해당 이슈 시점 기준
- 관련 스킬/도구: openclaw
- OS: 다양

## 출처
https://github.com/openclaw/openclaw/issues/50074
