...
source: https://github.com/openclaw/openclaw/issues/33809
---

# RFC: Self-Healing Tool Router — Deterministic Fallback Routing for Tool Failures

## 증상
Add a deterministic fallback routing layer to OpenClaw's tool execution pipeline. When a tool fails mid-execution, the router checks a fallback table before returning the error to the LLM. Predictable failures (timeouts, network errors, permission issues) get deterministic recovery. The LLM is only invoked when no feasible fallback exists.
