...
## 증상
When a sub-agent makes a POST request that succeeds (HTTP 201) but the TCP connection drops before the client receives the response, the retry path re-POSTs the same content, creating duplicate comments/actions.
