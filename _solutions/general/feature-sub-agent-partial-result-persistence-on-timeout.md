---
layout: solution
title: "[Feature]: Sub-agent partial result persistence on timeout"
category: general
---

# [Feature]: Sub-agent partial result persistence on timeout

## 증상
When a sub-agent times out, all output is lost. No partial results, no intermediate log. Especially painful for long-running SSH/build tasks.



## 원인
원본 이슈에서 확인 필요. GitHub Issue #37250 참조.

## 해결법
이 이슈의 해결법은 원본 GitHub Issue를 참조하세요.

## 예상 토큰 절약
이 에러로 삽질 시: 약 5,000~15,000 토큰 소비
이 해결법 참조 시: 약 500 토큰

## 환경
- OpenClaw 버전: 해당 이슈 시점 기준
- 관련 스킬/도구: general
- OS: 다양

## 출처
https://github.com/openclaw/openclaw/issues/37250
