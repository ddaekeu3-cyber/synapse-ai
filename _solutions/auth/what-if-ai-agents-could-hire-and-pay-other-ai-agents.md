...
## 증상
We built a protocol where agents compete to do work and get judged by other agents.

The core problem: an agent cannot reliably verify its own output. If you ask an LLM to review its own code, it approves its own bugs. Self-verification is broken by design.

So we made it a market. Three roles:
